 ...
 * - Truly parallel date fetching with high concurrency (10 concurrent requests)
 * - No sequential wave processing - all dates fetch simultaneously
 * - Prioritizes nearest dates first for better UX while maintaining parallelism
 * - Optimized for high-scale usage with thousands of concurrent users
 */
...
export function useDatePrices(
  ...
  const fetchDatePricesBatch = useCallback(async (indices: number[], type: 'departure' | 'return') => {
    ...
    // Increased concurrency for better parallelism and scalability
    const MAX_CONCURRENCY = 10; // Higher limit for truly parallel fetching

    // Process ALL dates in parallel with controlled concurrency
    // No sequential waves - all distance groups run concurrently
    const allIndicesToFetch = indicesToFetch.sort((a, b) => {
      // Sort by distance from center (nearest first) for better UX
      const distA = Math.abs(a - centerIndex);
      const distB = Math.abs(b - centerIndex);
      if (distA !== distB) return distA - distB;
      return a - b; // Stable sort for same distance
    });

    console.log(`[useDatePrices] PARALLEL FETCH ${type} indices=${JSON.stringify(allIndicesToFetch)} (max concurrency: ${MAX_CONCURRENCY})`);

    // Create a pool of workers that process requests in parallel
    let cursor = 0;
    const fetchOne = async (): Promise<void> => {
      while (cursor < allIndicesToFetch.length) {
        if (abortControllerRef.current?.signal.aborted) return;
        const idx = allIndicesToFetch[cursor++];
        if (idx !== undefined) {
    ...
    const workerCount = Math.min(MAX_CONCURRENCY, allIndicesToFetch.length);
